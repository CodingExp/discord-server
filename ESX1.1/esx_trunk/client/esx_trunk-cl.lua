 ...
 --   print('vPlate: ',vPlate)
  ...
  if not Config.CheckOwnership or (Config.AllowPolice and PlayerData.job.name == 'police') or (Config.CheckOwnership and myVeh) then  
        ...
        if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
          ...
          if locked == 1 or class == 15 or class == 16 or class == 14 then
           -- print('Trunk locked or improper class.')
            ...
            if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
              ...
              if inArray(model, Config.VehicleModel) then
               ...
               -- ESX.ShowNotification('weight is '..vehCapacity)
                TriggerEvent("pNotify:SendNotification", {text = 'Gewichtslimit für dieses Fahrzeug ist '..vehCapacity..'g.', type = "info", timeout = math.random(2000, 2000), layout = "bottomRight"})
              else
            ...
            TriggerEvent("pNotify:SendNotification", {text = _U('trunk_closed'), type = "info", timeout = math.random(1000, 1000), layout = "bottomRight"})
            --print('Trunk has been closed.')
          end
        end
      else
        --ESX.ShowNotification(_U('no_veh_nearby'))
        TriggerEvent("pNotify:SendNotification", {text = _U('no_veh_nearby'), type = "info", timeout = math.random(1000, 1000), layout = "bottomRight"})
   ...
   -- ESX.ShowNotification(_U('nacho_veh'))
    TriggerEvent("pNotify:SendNotification", {text = _U('nacho_veh'), type = "info", timeout = math.random(1000, 1000), layout = "bottomRight"})
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              if quantity == nil or quantity < 1 then
                ...
                TriggerEvent("pNotify:SendNotification", {text = _U('invalid_quantity'), type = "error", timeout = math.random(1000, 1000), layout = "bottomRight"})
              else
...
RegisterNetEvent("esx_trunk:openmenu")
    AddEventHandler("esx_trunk:openmenu", function()
            openmenuvehicle()
    end)