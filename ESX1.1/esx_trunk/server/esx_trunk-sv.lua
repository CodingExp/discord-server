        ...
        if coffre[i].name == item then
          if (coffre[i].count >= count and count > 0) then
            ...
            if xItem.count + count <= xItem.limit then
              ...
              TriggerClientEvent('esx:showNotification', _source, 'nicht genung Platz im Inventar für: '..xItem.label)
            end
            
            break
          else
			
            TriggerClientEvent("pNotify:SendNotification", _source, {text = _U('invalid_quantity'), type = "error", timeout = math.random(1000, 1000), layout = "bottomRight"})
  ...
  if type == 'item_account' then
    ...
    TriggerEvent('esx_trunk:getSharedDataStore', plate, function(store)
      ...
      if (blackMoney[1].amount >= count and count > 0) then
        ...
        TriggerClientEvent("pNotify:SendNotification", _source, {text = _U('invalid_amount'), type = "error", timeout = math.random(1000, 1000), layout = "bottomRight"})
      end
    end)
    ...
    if (playerItemCount >= count and count > 0 )then
      ...
      TriggerEvent('esx_trunk:getSharedDataStore', plate, function(store)
        ...
        if (getTotalInventoryWeight(plate)+(getItemWeight(item)*count))>max then
            ...
            TriggerClientEvent("pNotify:SendNotification", _source, {text = _U('insufficient_space'), type = "error", timeout = math.random(1000, 1000), layout = "bottomRight"})
        else
